...
| 2026-06-06 | Claude (Opus 4.8) | star6e (stab-fill port, commit 3) / imx335 | PASS w/ caveat | **T7 — stab-fill floating-image preset.** `framing=stab-fill` + `stabCropPct=80`: `stab-fill: manual-drain mode (precrop 2560x1920, encode 1440x1080, max_off 144x108)`, `sw_detect active (384x384 x2)` (IVE/blit threads pipelined), Kalman detector ticking. (1) Encode stays at full **1440x1080** (no shrink); the floating image rides on a black border bounded by `max_off` = `src*(100-cropPct)/200`. (2) **60 fps** steady (better than the spec's ~43 estimate — threaded blit overlaps IVE/compose on the dual-core A7). (3) **pauseStab** (D13 software ramp) device-verified: `PAUSED — gliding to centre` / `RESUMED`, fps stays 60, **no HW rebind**, no respawn (MUT_LIVE), dmesg clean — the board-wedge maneuver is gone. (4) Clean teardown/respawn `off↔stab-fill`, dmesg clean. Stabilization confirmed working on the groundstation. **D16 OSD (resolved):** the venc debug OSD composites on VPE port0 (the pre-compose source), so under stab-fill it rides the stabilized content — a known cosmetic limitation of this **vehicle-local dev diagnostic** (off by default).  Attempts to make it screen-fixed were dropped: RGN-on-VENC doesn't composite with manual-fed input on this BSP, the VPE counter-shift was lag-prone, and RGN-on-SCL works but shares the single global `MI_RGN` device with **waybeam_hub's `osd_render`** so the two are mutually exclusive (`MI_RGN_Init` fails for the second).  **The production static OSD is waybeam_hub** (composites at SCL post-shift, device-verified static over the stabilized stream); venc's debug OSD reverts to plain VPE.  Run hub OSD **or** venc debugOSD, not both. **CONFIG NOTE:** a stab preset only engages when `stabCropPct` ∈ [50,100]; this is now hardened — load + API + module clamp `stabCropPct` to **[60,100]** for stab presets, which also self-heals a stale `stabCropPct=0` (saved while `framing=off`, then framing set via `json_cli` string-only) that would otherwise silently fall through to the unstabilized default bind. |

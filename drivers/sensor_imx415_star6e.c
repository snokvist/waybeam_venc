...
// 3840x1152@60fps — ULTRAWIDE (full sensor WIDTH, letterbox) non-binned 1485.
// Reuses idx1's 1485 non-binned base; window widened to full 3840 and height
// cropped to 1152 (10:3 ~3.33:1).  HMAX=1022 (idx0's proven full-width value,
// above the wide-line analog floor), VMAX=1211 => 60fps; 1211 >= 1152 phys +
// vblank clears the vertical wall.  ISP 3840*1152*60 = 265 MPix/s < ~300.
// Warm-safe (writes non-binned + all-pixel DIG_CLP)
const static I2C_ARRAY Sensor_uw_3840x1152_60_init_table_4lane_linear[] = {
    { 0x3000, 0x01 }, // Standby
    { 0x3002, 0x01 }, // Master mode stop
    { 0x3008, 0x5D }, // BCWAIT_TIME[9:0]
    { 0x300A, 0x42 }, // CPWAIT_TIME[9:0]
    ...
    { 0x30DA, 0x02 }, // DIG_CLP_VNUM (all-pixel)
    { 0x301C, 0x04 }, // WINMODE (cropping mode)
    { 0x3024, 0xBB }, // VMAX=0x04BB=1211 (60fps @ HMAX=1022)
    { 0x3025, 0x04 }, //
    { 0x3028, 0xFE }, // HMAX=0x03FE=1022 (full-width line, from idx0)
    { 0x3029, 0x03 }, //
    { 0x3031, 0x00 }, // ADBIT (10bit)
    { 0x3032, 0x00 }, //
    { 0x3033, 0x08 }, // SYS_MODE
    ...
    { 0x3043, 0x0F }, //
    { 0x3044, 0xF0 }, // PIX_VST=0x03F0=1008 (centered)
    { 0x3045, 0x03 }, //
    { 0x3046, 0x00 }, // PIX_VWIDTH=0x0900=2304 (=1152 lines)
    { 0x3047, 0x09 }, //
    { 0x3050, 0x08 }, // SHR0[19:0]
    { 0x30C1, 0x00 }, // XVS_DRV[1:0]
    { 0x3116, 0x23 }, // INCKSEL2
...
// 1728x972@100fps — 2x2-BINNED WIDE CROP (soft, ~80% of full-FOV linear).
// Clone of the working 1472x816@120 binned crop (idx5): identical 891-link
// analog/PLL/MIPI/binning setup, WINMODE=0x04 crop with an EXPLICIT centered
// window (0x3040-47) — the full-FOV binned tables (idx2/idx4) lack a valid
// crop window and render black+colored-lines on the I6E ISP.  Widened window:
// HWIDTH=3464 (->1728 out), VWIDTH=3912 (->972 out), centered on the same
// optical axis (x~1944, v~2192).  100fps: HMAX=365 (unchanged, above the
// narrower-line analog floor), VMAX=2034 (>= 2*972 phys + 90 vblank).
const static I2C_ARRAY Sensor_bc1728_100fps_init_table_4lane_linear[] = {
    { 0x3000, 0x01 }, // Standby
    { 0x3002, 0x01 }, // Master mode stop
    { 0x3008, 0x5D }, // BCWAIT_TIME[9:0]
    { 0x300A, 0x42 }, // CPWAIT_TIME[9:0]
    ...
    { 0x3021, 0x01 }, // VADD (vertical binning)
    { 0x3022, 0x01 }, // ADDMODE (binning 2/2)
    { 0x3024, 0xF2 }, // VMAX=0x07F2=2034
    { 0x3025, 0x07 }, //
    { 0x3028, 0x6D }, // HMAX=0x016D=365
    { 0x3029, 0x01 }, //
    { 0x3031, 0x00 }, // ADBIT (10bit)
    { 0x3033, 0x05 }, // SYS_MODE (891Mbps)
    { 0x3040, 0xD4 }, // PIX_HST=0x00D4=212
    { 0x3041, 0x00 }, //
    { 0x3042, 0x88 }, // PIX_HWIDTH=0x0D88=3464
    { 0x3043, 0x0D }, //
    { 0x3044, 0xEC }, // PIX_VST=0x00EC=236
    { 0x3045, 0x00 }, //
    { 0x3046, 0x48 }, // PIX_VWIDTH=0x0F48=3912
    { 0x3047, 0x0F }, //
    { 0x3050, 0x08 }, // SHR0[19:0]
    { 0x30C1, 0x00 }, // XVS_DRV[1:0]
    { 0x30D9, 0x02 }, // DIG_CLP_VSTART (binning 2/2)
...
// 1728x816@120fps — 2x2-BINNED WIDE CROP.  Same 1728-wide window as the
// device-verified bc1728@100 mode; height trimmed to 816 so the frame fits
// 120fps at the PROVEN HMAX=365 (idx5's value; a 3464-col WINMODE=0x04 crop
// needs HMAX>=~365 — HMAX=308 rendered black+colored-lines).  VMAX=1700
// (>= 2*816 phys + 68 vblank), identical timing to idx5.  17% wider FOV than
// the 1472x816 idx5 crop.  169 MPix/s.
const static I2C_ARRAY Sensor_bc1728_120fps_init_table_4lane_linear[] = {
    { 0x3000, 0x01 }, // Standby
    { 0x3002, 0x01 }, // Master mode stop
    { 0x3008, 0x5D }, // BCWAIT_TIME[9:0]
    { 0x300A, 0x42 }, // CPWAIT_TIME[9:0]
    ...
    { 0x3021, 0x01 }, // VADD (vertical binning)
    { 0x3022, 0x01 }, // ADDMODE (binning 2/2)
    { 0x3024, 0xA4 }, // VMAX=0x06A4=1700
    { 0x3025, 0x06 }, //
    { 0x3028, 0x6D }, // HMAX=0x016D=365 (proven crop-floor-clear at 1728 wide)
    { 0x3029, 0x01 }, //
    { 0x3031, 0x00 }, // ADBIT (10bit)
    { 0x3033, 0x05 }, // SYS_MODE (891Mbps)
    { 0x3040, 0xD4 }, // PIX_HST=0x00D4=212
    ...
    { 0x3042, 0x88 }, // PIX_HWIDTH=0x0D88=3464
    { 0x3043, 0x0D }, //
    { 0x3044, 0x24 }, // PIX_VST=0x0224=548 (816-tall crop)
    { 0x3045, 0x02 }, //
...
static int pCus_init_uw_3840x1152_60_mipi4lane_linear(ms_cus_sensor* handle)
{
    int i, cnt = 0;
    ...
    for (i = 0; i < ARRAY_SIZE(Sensor_uw_3840x1152_60_init_table_4lane_linear); i++) {
        if (Sensor_uw_3840x1152_60_init_table_4lane_linear[i].reg == 0xffff) {
            SENSOR_MSLEEP(Sensor_uw_3840x1152_60_init_table_4lane_linear[i].data);
        } else {
            cnt = 0;
...
static int pCus_init_bc1728_100fps_mipi4lane_linear(ms_cus_sensor* handle)
{
    int i, cnt = 0;
    ...
    for (i = 0; i < ARRAY_SIZE(Sensor_bc1728_100fps_init_table_4lane_linear); i++) {
        if (Sensor_bc1728_100fps_init_table_4lane_linear[i].reg == 0xffff) {
            SENSOR_MSLEEP(Sensor_bc1728_100fps_init_table_4lane_linear[i].data);
        } else {
            cnt = 0;
            while (SensorReg_Write(Sensor_bc1728_100fps_init_table_4lane_linear[i].reg, Sensor_bc1728_100fps_init_table_4lane_linear[i].data) != SUCCESS) {
                cnt++;
                if (cnt >= 10) {
                    SENSOR_EMSG("[%s:%d]Sensor init fail!!\n", __FUNCTION__, __LINE__);
                    return FAIL;
...
static int pCus_init_bc1728_120fps_mipi4lane_linear(ms_cus_sensor* handle)
{
    int i, cnt = 0;
    ...
    for (i = 0; i < ARRAY_SIZE(Sensor_bc1728_120fps_init_table_4lane_linear); i++) {
        if (Sensor_bc1728_120fps_init_table_4lane_linear[i].reg == 0xffff) {
            SENSOR_MSLEEP(Sensor_bc1728_120fps_init_table_4lane_linear[i].data);
        } else {
            cnt = 0;
            while (SensorReg_Write(Sensor_bc1728_120fps_init_table_4lane_linear[i].reg, Sensor_bc1728_120fps_init_table_4lane_linear[i].data) != SUCCESS) {
                cnt++;
                if (cnt >= 10) {
                    SENSOR_EMSG("[%s:%d]Sensor init fail!!\n", __FUNCTION__, __LINE__);
                    return FAIL;
...
static int pCus_SetVideoRes(ms_cus_sensor* handle, u32 res_idx)
{
    ...
    case 6: // 1728x972@100 — 2x2-binned WIDE CROP (clean), HMAX=365, VMAX=2034
        ...
        params->expo.vts = vts_30fps;
        params->expo.fps = 100;
        Preview_line_period = 4882; // HMAX=365 line;
        handle->data_prec = CUS_DATAPRECISION_12;
        break;
...
static int cus_camsensor_init_handle_hdr_dol_sef(ms_cus_sensor* drv_handle)
{
    ms_cus_sensor* handle = drv_handle;
 ...
 * linear (fps-ordered) lineup is registered.  With the SEF handle also made
 * static (see its definition), the compiler dead-code-eliminates the whole
 * HDR init/AE subtree.  Mirrors the IMX335 Star6E driver. */

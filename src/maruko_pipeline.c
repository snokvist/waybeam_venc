...
static int bind_maruko_pipeline(MarukoBackendContext *ctx)
{
	...
	/* The grayscale tap programs SCL port3 itself, so it needs the same crop
	 * window the other taps are cut from — and takes its size as the tap's
	 * full-resolution default (the active sensor mode post-precrop). */
	venc_jpeg_set_gray_source(ctx->scl_crop_x, ctx->scl_crop_y,

...
mode 4 (1920×1080@120).  Awaiting bench verification on .13.
...
- Verification note: cross-toolchain download is egress-blocked in the dev
  environment this change was built in; verified via host `-Werror` syntax
  lint of both backends + full host unit suite (1967 tests, incl. ASan).
  `make verify` + the .13 bench plan (investigation doc §6) still pending.
